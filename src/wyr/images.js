...
const STOP_WORDS = new Set(['a', 'an', 'the', 'and', 'or', 'to', 'of', 'in', 'on', 'at', 'with', 'for', 'your', 'you', 'become', 'be', 'own', 'have', 'anywhere', 'through']);
...
export const buildImageQueries = option => {
  ...
  const has = word => optionWords.includes(word); let visualQuery = '';
  if (has('minds') || has('mind')) visualQuery = 'two people telepathy glowing thoughts mind reading cinematic';
  else if (has('future')) visualQuery = 'person seeing future crystal ball vision cinematic';
  else if (has('teleport')) visualQuery = 'person entering glowing teleportation portal';
  else if (has('invisible') || has('invisibility')) visualQuery = 'invisible man empty clothes disappearing cinematic scene';
  else if (has('stop') && has('time')) visualQuery = 'frozen city people stopped in time cinematic scene';
  else if (has('travel') && has('time')) visualQuery = 'person walking through time portal cinematic scene';
  else if (has('dragon')) visualQuery = 'person petting friendly dragon fantasy art';
  else if (has('portal') && has('door')) visualQuery = 'doorway opening into another world fantasy cinematic';
  return [...new Set([
    visualQuery,
...
const explicitVisualIntent = (option, tokens) => {
  const words = normalizeWords(option.text); const has = word => words.includes(word);
  const groups = [];
  if (has('minds') || has('mind')) groups.push(['person', 'people', 'human', 'head', 'face', 'minds'], ['telepathy', 'telepathic', 'thoughts', 'mind', 'brain', 'psychic']);
  ...
  return groups.length === 0 || groups.every(group => containsAny(tokens, group));
};
export const assessImageCandidate = (candidate, option) => {
  ...
  return { accepted: rejectionReasons.length === 0, validAsset: assetRejectionReasons.length === 0, relevanceScore, rejectionReasons, matchedConcepts: uniqueWords(coreMatched) };
};
...
const collectCandidateJobs = async ({ jobs, provider, providerLabel, concurrency, retrySearch }) => {
  ...
  for (const result of results) {
    ...
    for (const candidate of candidates) {
      ...
      const enriched = { ...candidate, provider: candidate.provider || providerLabel, query, relevanceScore: assessment.relevanceScore, matchedConcepts: assessment.matchedConcepts };
...
const rankedUnique = candidates => {
  ...
  for (const candidate of candidates) {
    ...
    if (!current || candidate.relevanceScore > current.relevanceScore) byIdentity.set(identity, candidate);
  }
  return [...byIdentity.values()].sort((left, right) => right.relevanceScore - left.relevanceScore || left.position - right.position);
};
...
export const findAndDownloadImages = async ({ plan, provider, webProvider = null, assetsDir, maxRetries, concurrency = 4, onProgress }) => {
  ...
  for (const state of states) { state.pexelsCandidates = rankedUnique(state.candidates.filter(candidate => candidate.provider === 'Pexels')).slice(0, MAX_RANKED_CANDIDATES); state.pexelsFallbackCandidates = rankedUnique(state.validCandidates.filter(candidate => candidate.provider === 'Pexels')).slice(0, MAX_RANKED_CANDIDATES); state.pexelsGatePassed = state.pexelsCandidates.length > 0; }
  ...
  if (webProvider && needsWeb.length) {
    ...
    for (const state of needsWeb) state.webCandidates = rankedUnique(state.candidates).slice(0, MAX_RANKED_CANDIDATES);
  }
  for (const state of needsWeb) {
    state.pool = [...state.webCandidates, ...state.pexelsFallbackCandidates];
