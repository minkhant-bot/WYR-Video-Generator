  ...
  while (attempt < maxAttempts && accepted.length < questionCount) {
    ...
    try {
      ...
      if (rateLimit.rateLimited) {
        if (rateLimitRetries >= policy.maxRetries) { log('content.groq_rate_limit_exhausted', { retries: rateLimitRetries, maxRetries: policy.maxRetries, cumulativeWaitMs: rateLimitWaitedMs, maxWaitMs: policy.maxWaitMs, accepted: accepted.length, required: questionCount, ...providerRequestStats(provider) }); throw rateLimitFailure({ accepted, questionCount, retries: rateLimitRetries, waitedMs: rateLimitWaitedMs, maxRetries: policy.maxRetries, maxWaitMs: policy.maxWaitMs, lastProviderError }); }
        ...
        if (!Number.isFinite(waitMs) || waitMs < 0 || rateLimitWaitedMs + waitMs > policy.maxWaitMs) { log('content.groq_rate_limit_exhausted', { retries: rateLimitRetries, maxRetries: policy.maxRetries, cumulativeWaitMs: rateLimitWaitedMs, maxWaitMs: policy.maxWaitMs, accepted: accepted.length, required: questionCount, ...providerRequestStats(provider) }); throw rateLimitFailure({ accepted, questionCount, retries: rateLimitRetries, waitedMs: rateLimitWaitedMs, maxRetries: policy.maxRetries, maxWaitMs: policy.maxWaitMs, lastProviderError }); }
        rateLimitRetries += 1; rateLimitWaitedMs += waitMs;
        log('content.groq_rate_limit_wait', { retry: rateLimitRetries, maxRetries: policy.maxRetries, retryAfterMs: rateLimit.retryAfterMs, waitMs, cumulativeWaitMs: rateLimitWaitedMs, accepted: accepted.length, required: questionCount, remaining: questionCount - accepted.length, source: rateLimit.retryAfterMs === null ? 'exponential_backoff' : 'retry_after', ...providerRequestStats(provider) });
