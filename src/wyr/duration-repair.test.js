...
// long-narration questions, one per distinct content family, whose combined ESTIMATED duration
// reproduces the production near-60s class of failure -- long enough to clear the budget target,
// short enough to still pass computeInsertionFields' own quality gate.
const LONG_QUESTIONS = [
  question('superpowers', 'Read the mind of every single stranger you see', 'telepathy mind reading glow', 'Turn fully invisible near every other person you see', 'invisible person disappearing'),
  question('time', 'Travel back to relive your whole early childhood again', 'child playing park sunny', 'Travel forward to see the far unknown future today', 'futuristic city scene glowing'),
  question('dream lifestyle', 'Wake up early and productive every single busy morning', 'sunrise desk productive morning', 'Sleep in late and relaxed every single lazy day', 'person sleeping bed morning'),
  question('food', 'Eat wildly spicy curry for every single family meal', 'spicy curry bowl closeup', 'Bake sweet cakes for every big family event today', 'sweet pastries bakery display'),
  question('travel', 'Backpack through five busy cities in western Europe now', 'backpacker european street cobblestone', 'Sail through six remote tropical isles near Asia now', 'tropical islands ocean boat'),
  question('space', 'Float weightless inside a real orbiting space ship soon', 'astronaut floating zero gravity', 'Walk slowly across the dusty surface of the moon', 'moon surface astronaut walking'),
];
...
test('a long, over-budget 6-question DB selection is automatically repaired in place, stays under budget, and never calls Groq', () => withFakeDb(async fake => {
  ...
  try {
    const reservation = await selectAndReservePlan({ jobId: 'job-repair-1' }); // production default count (6)
...
test('Scene 1 is the strongest hook among the POST-repair 6, not whichever question happened to be selected first', () => withFakeDb(async fake => {
  await insertQuestions(LONG_QUESTIONS);
  await insertQuestions(SHORT_QUESTIONS);
  markFreshest(fake, new Set(LONG_QUESTIONS.map(q => q.optionA.text)), 0);
  markFreshest(fake, new Set(SHORT_QUESTIONS.map(q => q.optionA.text)), 1);

  const plan = await selectPlanForJob({ jobId: 'job-repair-hook' }); // production default count (6)
  assert.ok(plan);
  ...
  // among THOSE 6 -- not merely the highest among the original pre-repair 6, which duration repair
  ...
  assert.equal(Number(scene1Row.hook_score), highestHookScore, 'Scene 1 must carry the highest hook_score among the final, post-repair 6 questions');
}));
...
test('when no valid local substitute exists at all, selectAndReservePlan throws DurationBudgetExceededError (not CONTENT_POOL_EMPTY) and reserves nothing', () => withFakeDb(async fake => {
  ...
  try {
    await assert.rejects(() => selectAndReservePlan({ jobId: 'job-repair-2' }), error => { // production default count (6)
...
test('selectContentPlan (the normal video-generation entry point) propagates a duration-budget failure without ever calling Groq, even when a Groq key is configured', () => withFakeDb(async fake => {
  ...
  globalThis.fetch = async () => { fetchCalled = true; throw new Error('must not call Groq'); };
  try {
    const config = { questionCount: 6, groqApiKey: 'test-key', groqModel: 'openai/gpt-oss-20b', timeoutMs: 1000, poolTarget: 10, poolLowWaterMark: 100, poolEmergencyRefillMaxBatches: 1, secondsPerQuestion: 7 };
