...
const config = overrides => ({ questionCount: 6, groqApiKey: '', groqModel: 'openai/gpt-oss-20b', timeoutMs: 1000, poolTarget: 10, poolLowWaterMark: 100, poolEmergencyRefillMaxBatches: 1, ...overrides });

test('when the DB pool already has 6 valid questions, no live Groq request is made', () => withFakeDb(async () => {
...
test('an empty pool with a configured Groq key attempts exactly one bounded emergency refill, then succeeds', () => withFakeDb(async () => {
  ...
  globalThis.fetch = async () => {
    calls += 1;
    const plan = { questions: SIX_DIVERSE };
