...
export const createJobStore = rootDir => {
  ...
  return {
    create() { const id = crypto.randomUUID(); const workspace = path.join(rootDir, id); for (const folder of ['', 'assets', 'audio', 'render', 'output']) fs.mkdirSync(path.join(workspace, folder), { recursive: true }); const now = new Date().toISOString(); const job = { id, workspace, status: 'queued', stage: 'queued', progress: 0, error: null, topic: null, createdAt: now, updatedAt: now, verification: null }; jobs.set(id, job); persist(job); return job; },
